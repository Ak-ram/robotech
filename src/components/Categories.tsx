    ...
    <div className=" w-64 pb-10">
      <div className="flex h-full flex-grow flex-col overflow-y-auto rounded-br-lg rounded-tr-lg bg-white shadow-md">
        ...
        <div className="flex flex-col">
          ...
          <nav className="flex-1">
            {categories?.map((cat_title: string, i: number) => (
              <button
                ...
                className={`w-full ${selectedCategory === cat_title ? 'bg-gray-100 text-black' : 'text-zinc-500 hover:bg-zinc-100 hover:text-zinc-600'} flex cursor-pointer items-center border-l-designColor py-2 px-4 text-sm font-medium text-gray-600 outline-none transition-all duration-100 ease-in-out hover:border-l-4 hover:border-l-designColor hover:text-designColor focus:border-l-4`}>
          ...
          <span className="ml-3 mt-10 mb-2 block text-xs font-semibold text-gray-500">Pages</span>

          <nav className="flex-1">
            {navigation.map((item) => (
              <Link href={item?.href} key={item._id} className={`hover:bg-zinc-100 flex cursor-pointer items-center border-l-designColor py-2 px-4 text-sm font-medium text-gray-600 outline-none transition-all duration-100 ease-in-out hover:border-l-4 hover:border-l-designColor hover:text-designColor-600 focus:border-l-4`}>
                <svg className="mr-4 h-5 w-5 align-middle" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
