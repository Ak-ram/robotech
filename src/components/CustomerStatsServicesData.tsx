...
const CustomerStatsServicesData = () => {
  ...
  const [selectedService, setSelectedService] = useState<ProductType | null>(
    null
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setServices(data!); // Initial sorting based on sortOrder
        setJsonData(data!);
      } catch (error) {
  ...
  const handleSave = async () => {
    if (editedService) {
      ...
      if (jsonData.length > 0) {
        ...
        console.error(
          "jsonData is empty or does not contain the expected structure."
        );
      }
    }
  };
        ...
        >
          <h2 className="whitespace-nowrap font-semibold  flex items-center justify-center text-indigo-500 bg-indigo-100 py-2 px-4 rounded-md">
            ...
            <span className="ml-auto text-sm">
              {services && services.length} Items(s)
            </span>
          </h2>
                    ...
                    <div key={key} className={`flex-col mb-2 lg:pr-4`}>
                      ...
                      {key === "description" ? (
                        <textarea
                          ...
                          onChange={(e) =>
                            handleInputChange(e, key as keyof ProductType)
                          }
                        />
                      ) : (
                        <input
                          type={
                            key === "price" || key === "wholesalePrice"
                              ? "number"
                              : "text"
                          } // Set the type for price and wholesalePrice to 'number'
                          placeholder={placeholder}
                          className={`border border-indigo-300 rounded outline-none w-full p-2 `}
                          value={editedService[key]}
                          onChange={(e) =>
