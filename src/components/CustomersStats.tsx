...
const CustomersStats = () => {
  ...
  const sortCustomers = (data: any[], order: "asc" | "desc") => {
    const sortedCustomers = [...data].sort((a, b) => {
      const compareValue =
        b.total_purchase_transactions - a.total_purchase_transactions;
    ...
    const highestTotal = sortedCustomers.reduce((max, customer) => {
      return customer.total_purchase_transactions > max
        ? customer.total_purchase_transactions
        : max;
    }, 0);
    ...
    <>
      <section className="my-5">
        ...
        <div className="flex flex-col">
          ...
          <div
            className={`${
              !show ? "border border-blue-300 border-dashed" : ""
                ...
                <div className="shadow overflow-hidden sm:rounded-lg">
                  <div className="flex justify-between items-center gap-2 bg-gray-800 p-3 border-b  border-gray-700">
                    ...
                    <div className="flex items-center gap-1">
                      <span className="text-white text-xs">
                        {filteredCategoryStats?.length} Customer
                      </span>
                      <span
                        className="text-slate-400 hover:text-white cursor-pointer py-1 rounded w-10 h-10 flex items-center justify-center  hover:bg-gray-700 block"
                  ...
                  <div className="w-full max-h-[400px] overflow-auto">
                    <div className="bg-gray-800 uppercase text-gray-400">
                      <div className="flex">
                        <div className="p-3 flex-1 text-left text-sm tracking-wider">
                          Name
                        </div>
                        <div className="p-3 w-1/6 text-left text-sm tracking-wider">
                          Phone
                        </div>
                        <div className="p-3 w-1/6 text-left text-sm tracking-wider">
                          # Transactions
                    ...
                    <div className="bg-gray-800 text-white">
                      {filteredCategoryStats.map((customerInfo, index) => (
                        <div
                          key={index}
                          className="flex hover:bg-opacity-50 bg-black bg-opacity-20"
                        >
                          <div className="p-3 flex gap-2 flex-1 whitespace-nowrap">
                            {highestTotalPurchase ===
                              customerInfo.total_purchase_transactions && (
                              <span title="عميل مميز - الاكثر شراء">
                                <Sparkle
                                  className="mt-1 text-yellow-500 mr-2 animate-pulse"
                                  size={20}
                                />
                              </span>
                            )}
                            {calculateNumberOfTransactions(customerInfo.id) >
                              ...
                              <span title="عميل متكرر">
                                <Activity
                                  className="mt-1 text-green-500 mr-2 animate-pulse"
                                  size={20}
                                />
                              </span>
                            )}
                            {calculateNumberOfTransactions(customerInfo.id) ===
                              ...
                              <span title="عميل عابر">
                                <GitCommitHorizontal
                                  className="mt-1 text-rose-500 mr-2 animate-pulse"
                                  size={20}
                                />
                              </span>
                            )}
                            {calculateNumberOfTransactions(customerInfo.id) ===
                              ...
                              <span title="عميل محتمل">
                                <Clock
                                  className="mt-1 text-slate-500 mr-2 animate-pulse"
                                  size={20}
                                />
                              </span>
                            )}
                          ...
                          <div className="p-3 w-1/6 whitespace-nowrap">
                            <FormattedPrice
                              amount={customerInfo.total_purchase_transactions}
                            />
                          </div>
                          <div className="p-3 w-1/6 whitespace-nowrap">
                            <Link
