...
const TransactionAnalyzer = ({ customers }) => {
  ...
  customers.forEach((customer) => {
    customer.transactions.products.forEach((transaction) => {
      ...
      dailyProfits[dayKey] += parseInt(
        (
          transaction.subtotal -
          (transaction.wholesalePrice * transaction.quantity || 0)
        ).toString(),
      );
      ...
      monthlyProfits[monthKey] += parseInt(
        (
          transaction.subtotal -
          (transaction.wholesalePrice * transaction.quantity || 0)
        ).toString(),
      ...
      yearlyProfits[yearKey] += parseInt(
        (
          transaction.subtotal -
          (transaction.wholesalePrice * transaction.quantity || 0)
        ).toString(),
    ...
    customer.transactions.courses.forEach((course) => {
      ...
      dailyProfits[dayKey] += parseInt(
        (
          course.subtotal - (course.wholesalePrice * course.quantity || 0)
        ).toString(),
      );
    ...
    customer.transactions.printServices.forEach((printService) => {
      ...
      dailyProfits[dayKey] += parseInt(
        (
          printService.subtotal -
          (printService.wholesalePrice * printService.quantity || 0)
        ).toString(),
      );
      ...
      yearlyProfits[yearKey] += parseInt(
        (printService.subtotal - (printService.wholesalePrice || 0)).toString(),
      );
    });
  });
        ...
        return transactions.map((transaction, index) => {
          return (
            <Link
              ...
              key={index}
              className="flex hover:bg-slate-100 border border-slate-200 rounded p-3 mb-2 items-center gap-3"
            >
                ...
                <div className="ml-auto flex gap-4">
                  ...
                  >
                    ...
                    <FormattedPrice
                      ...
                      amount={+transaction?.wholesalePrice! || 0}
                    />
                  </div>
                  <div
                    title="السعر اللى هتبيع بيه"
                    className="flex  flex-col justify-center items-center"
                  >
                    <span className="text-xs font-semibold">Sell</span>
                    <FormattedPrice
                      className="text-xs font-semibold"
                      amount={+transaction.piecePrice}
                    />
                  </div>
                  <div
                    title="عدد القطع المباعه"
                    className="flex  flex-col justify-center items-center"
                  >
                  ...
                  >
                    ...
                    <FormattedPrice
                      className="text-xs font-semibold"
                      amount={+transaction.quantity * +transaction.piecePrice}
                    />
                  </div>
                  <div
                    title="قيمه الخصم"
                  ...
                  >
                    ...
                    <FormattedPrice
                      className="text-xs font-semibold"
                      amount={+transaction.discount || 0}
                    />
                  </div>
                  <div
                    title="صافى المبلغ او الفلوس الي انت اخذتها من العميل"
                    className="flex  flex-col justify-center items-center"
                  >
                  ...
                  {transaction.wholesalePrice !== undefined && (
                    <>
                      {+transaction.wholesalePrice * +transaction.quantity <
                        ...
                        >
                          <span className="text-xs flex items-center text-green-400 font-semibold">
                            Profit <TrendingUp className="ml-1" size={16} />
                          </span>
                          <FormattedPrice
                      ...
                      {+transaction.wholesalePrice * +transaction.quantity >
                        ...
                        <div
                          ...
                          className="flex flex-col items-center"
                        >
                          <span className="text-xs flex items-center text-red-400 font-semibold">
                            Loss <TrendingDown className="ml-1" size={16} />
                          </span>
                          <FormattedPrice
                      ...
                      {+transaction.wholesalePrice * +transaction.quantity >=
                        ...
                        +transaction.wholesalePrice * +transaction.quantity <=
                          ...
                          <div
                            ...
                            className="flex flex-col items-center"
                          >
                            <span className="text-xs flex items-center text-orange-400 font-semibold">
                              Fair <Diff className="ml-1" size={16} />
                            </span>
                            <FormattedPrice
              ...
              <div key={day} className="mb-4">
                <h3 className="text-lg flex items-center justify-between w-full font-semibold">
                  ...
                  <span className="text-xs ml-auto">
                    <FormattedPrice
                      className="text-xs"
                      amount={dailySells[day].reduce(
                        (accumulator, day) => accumulator + day.subtotal,
                        0,
                      )}
                    />{" "}
                    for {dailySells[day].length} Transaction(s)
                  </span>
                </h3>
              ...
              <div className="bg-white rounded-lg p-4 mb-4">
                <div className="flex items-center mb-4">
                  ...
                  <p className="ml-auto text-lg font-semibold">
                    <FormattedPrice amount={calculateTotalRevenue} />
                  </p>
                </div>
                <div>
                  <h4 className="text-md font-semibold mb-2">
                    Daily Revenue and Profits
                  </h4>
                  {Object.entries(dailyRevenue)
                    ...
                    .map(([day, revenue]) => (
                      <p key={day} className="flex items-center my-2">
                        ...
                        <span>
                          Revenue: <FormattedPrice amount={revenue as number} />
                        </span>
                        {dailyProfits[day] && (
                          <span className="ml-3">
                            Outcome:{" "}
                            <FormattedPrice
                              amount={dailyProfits[day] as number}
                            />
                          </span>
                        )}
                      </p>
                      ...
                      <p key={day} className="flex items-center my-2">
                        ...
                        {monthlyProfits[day] && (
                          <span className="ml-3">
                            Outcome:{" "}
                            <FormattedPrice
                              amount={monthlyProfits[day] as number}
                            />
                          </span>
                        )}
                      </p>
                      ...
                      <p key={day} className="flex items-center my-2">
                        ...
                        {yearlyProfits[day] && (
                          <span className="ml-3">
                            Outcome:{" "}
                            <FormattedPrice
                              amount={yearlyProfits[day] as number}
                            />
                          </span>
                        )}
                      </p>
  ...
  return (
    ...
    <div
      className={`${!show ? "border border-orange-500 border-dashed" : ""} bg-white  rounded-lg mb-5 overflow-hidden`}
    >
      <div
      ...
      {show && (
        <>
          ...
          <div className="px-5 grid grid-cols-1 md:grid-cols-4 gap-4">
            <div
              className="bg-white border border-indigo-500 rounded-lg shadow-lg p-4 flex items-center justify-center cursor-pointer"
            ...
            >
              ...
              <div>
                ...
                <p className="text-indigo-500">
                  {Object.keys(dailySells).length}
                </p>
              </div>
            </div>
            <div
              className="bg-white border border-green-500  rounded-lg shadow-lg p-4 flex items-center justify-center cursor-pointer"
            ...
            >
              ...
              <div>
                ...
                <p className="text-green-500 ">
                  {Object.keys(monthlySells).length}
                </p>
              </div>
            </div>
            <div
              className="bg-white rounded-lg border border-orange-500 shadow-lg p-4 flex items-center justify-center cursor-pointer"
            ...
            >
              ...
              <div>
                ...
                <p className="text-orange-600">
                  {Object.keys(yearlySells).length}
                </p>
              </div>
            </div>
            <div
              className="bg-white border border-rose-500 rounded-lg shadow-lg p-4 flex items-center justify-center cursor-pointer"
              onClick={() => handlePeriodClick("revenue")}
            >
              <Banknote className="w-8 h-8 mr-2 text-rose-500" />
              <div>
                <h3 className="text-lg text-rose-500 font-semibold">
                  Total Revenue
                </h3>
          ...
          <RevenueCharts
            monthlyProfitsData={monthlyProfitsData}
            yearlyProfitsData={yearlyProfitsData}
            dailyProfitsData={dailyProfitsData}
