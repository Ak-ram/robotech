      ...
      <div className="container mx-auto px-4 md:px-0">
        <div className="mb-10 text-center">
          ...
          <p className="text-base text-gray-700 md:text-lg">
            We integrate cutting-edge technologies for precision and innovation
            in spacecraft design, robotics, and control systems.
          </p>
        </div>
        <div className="grid gap-10 md:grid-cols-2">
          {data.map((item) => (
            ...
            >
              ...
              <div className="p-6">
                <div>
                  <h6 className="text-xl font-bold uppercase leading-5 text-gray-800 mb-3">
                    {item.title}
                  </h6>
                  <p className=" text-gray-700 mb-3">{item.description}</p>
                  <Link
                    href={item.link_url}
                    className="inline-block flex gap-1 items-center justify-center bg-slate-500 text-white py-2 px-4 rounded-md font-semibold transition-colors duration-200 hover:bg-slate-600"
                  >
                    <Link2 />
                    {item.link_text.charAt(0).toUpperCase() +
                      item.link_text.slice(1).toLowerCase()}
                  </Link>
                </div>
              </div>
            </div>
